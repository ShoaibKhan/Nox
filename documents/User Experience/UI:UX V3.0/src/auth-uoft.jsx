// Faithful recreation of the U of T weblogin screen (real screenshot).
// Layout: 3 columns — login | protect-your-account body | side tabs (right).
...
const UoftAuthScreen = ({utorid='khansh4', onSubmit}) => {
  const [id, setId] = React.useState(utorid);
  const [pw, setPw] = React.useState('••••••••••');
  const [tab, setTab] = React.useState('protect');
  const tabs = [
  ...
  return (
    <div style={{width:'100%', height:'100%', background:'#fff', fontFamily:'Arial, Helvetica, sans-serif', color:'#222', display:'flex', flexDirection:'column', overflow:'hidden'}}>
      {/* Crest header */}
      ...
      <div style={{display:'grid', gridTemplateColumns:'260px 1fr 200px', gap:0, padding:'18px 60px 30px', flex:1, minHeight:0}}>
        ...
        <div style={{padding:'0 18px 0 0'}}>
          ...
          <button onClick={onSubmit} style={{display:'inline-flex', alignItems:'center', gap:8, background:'#3A8FCB', color:'#fff', border:'none', padding:'7px 13px', fontSize:13, fontWeight:600, cursor:'pointer'}}>
            Log in
          ...
          <div style={{marginTop:8}}><a href="#" style={{fontSize:11, color:'#3A8FCB', textDecoration:'underline'}}>Forgotten Password?</a></div>
        ...
        <div style={{padding:'0 30px', borderLeft:'1px solid #e8e8e8'}}>
          ...
          <ul style={{listStyle:'none', padding:0, margin:0, fontSize:12.5, color:'#222', lineHeight:1.55}}>
            <li style={{display:'flex', gap:10, marginBottom:14, alignItems:'flex-start'}}>
              ...
              <span>Before you begin, make sure the weblogin page (URL) starts with: <a href="#" style={{color:'#3A8FCB'}}>https://weblogin.utoronto.ca/...</a></span>
            </li>
            <li style={{display:'flex', gap:10, marginBottom:14, alignItems:'flex-start'}}>
              <span style={{flexShrink:0, marginTop:2, width:16, height:16, borderRadius:'50%', background:'#3F8C42', display:'inline-flex', alignItems:'center', justifyContent:'center'}}>
                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="4"><polyline points="4 12 10 18 20 6"/></svg>
            ...
            <li style={{display:'flex', gap:10, marginBottom:14, alignItems:'flex-start'}}>
              ...
              <span>When using a public computer, <a href="#" style={{color:'#3A8FCB'}}>close all windows</a> and exit the browser.</span>
            </li>
            <li style={{display:'flex', gap:10, marginBottom:14, alignItems:'flex-start'}}>
              <span style={{flexShrink:0, marginTop:2, width:16, height:16, borderRadius:'50%', background:'#3F8C42', display:'inline-flex', alignItems:'center', justifyContent:'center'}}>
                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="4"><polyline points="4 12 10 18 20 6"/></svg>
              ...
              <div>
                ...
                <div style={{fontSize:11, color:'#555', marginTop:3}}>Tip: U of T will <strong style={{color:'#B23A2A'}}>never</strong> ask for your password or other personal information by e-mail.</div>
              </div>
            </li>
          </ul>
        </div>
          ...
          {tabs.map((t,i)=>{
            ...
            return (
              <button key={t.k} onClick={()=>setTab(t.k)} style={{
                ...
                borderBottom: 'none',
                padding:'7px 10px', fontSize:11.5, fontWeight: active?600:400,
      ...
      <div style={{padding:'10px 60px', borderTop:'1px solid #e1e1e1', background:'#fff', fontSize:11, color:'#666', textAlign:'right'}}>
        <a href="#" style={{color:'#3A8FCB'}}>Site Feedback</a> &nbsp; <a href="#" style={{color:'#3A8FCB'}}>Accessibility</a> &nbsp; © University of Toronto
      </div>
    </div>
  );
};

window.UoftAuthScreen = UoftAuthScreen;
window.UoftCrest = UoftCrest;
