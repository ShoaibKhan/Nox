// Shared visual primitives reused across professor + student artboards.

// A small chip using monospace -- matches existing Nox vocabulary
const Pill = ({children, tone='ink', style, ...rest}) => {
  const tones = {
...
// The U-of-T-styled top bar used by all "in-app" screens (mobile + desktop).
const NoxTopBar = ({left, center, right, dark=true, height=58, style}) => (
  <header style={{
...
const NoxLockup = ({size=22, color='currentColor', showWord=true}) => (
  <span style={{display:'inline-flex', alignItems:'center', gap:8, color}}>
    <NoxMark size={size} color={color} />
    {showWord && <span className="serif" style={{fontSize:size*0.85, letterSpacing:'0.08em', fontStyle:'italic', fontWeight:500}}>Nox</span>}
  </span>
);

// Small button used inline on dashboards
const Btn = ({children, kind='primary', size='md', icon, iconRight, style, ...rest}) => {
  const sizes = {
  ...
  return (
    <button style={{
      ...
      background:k.bg, color:k.fg, cursor:'pointer',
      transition:'background .15s ease, transform .05s ease',
      ...style
    }}
      onMouseDown={e=>e.currentTarget.style.transform='translateY(1px)'}
      onMouseUp={e=>e.currentTarget.style.transform='translateY(0)'}
      onMouseLeave={e=>{e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.background=k.bg}}
...
// Sparkline / pulse line for the live mood graph
const PulseLine = ({values, width=520, height=72, strokeColor='var(--uoft-navy)', fillColor='rgba(74,111,165,0.18)'}) => {
...
// Vertical bar chart for poll results (simple, accessible)
const PollBars = ({options, totalVotes, accent='var(--uoft-navy)'}) => {
        ...
        return (
          <div key={i} style={{position:'relative'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:4}}>
              ...
              <span className="mono" style={{fontSize:12, color:'var(--ink-3)'}}>{o.votes} · {pct}%</span>
            </div>
...
// Tiny histogram/dot pip used on pulse pulse buttons
const Dot = ({color='var(--uoft-navy)', size=8, style}) => (
  <span style={{display:'inline-block', width:size, height:size, borderRadius:'50%', background:color, ...style}}/>
);

// Sectioned label like "Live · CSC209H5"
...
Object.assign(window, {
  Pill, NoxTopBar, NoxLockup, Btn, Card, PulseLine, PollBars, Dot, SectionLabel
});
